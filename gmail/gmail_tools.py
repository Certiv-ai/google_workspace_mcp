        ...
        for mid in chunk_ids:
            ...
            if entry["error"]:
                ...
                if format == "metadata":
                    ...
                    # get_gmail_attachment_content. Use the shared helper so this
                    # matches get_gmail_message_content's formatting exactly. Only the
                    # "full" payload (text/html modes) carries part/attachmentId data;
                    # raw mode fetches metadata only, so `attachments` is empty there.
                    attachment_lines = _format_attachment_lines(attachments, mid)
                    if attachment_lines:
...
async def forward_gmail_message(
    ...
    """
    Forward a Gmail message, preserving its attachments by default.

    This downloads the original message (including attachments) and forwards it
    to the specified recipient. Attachments such as PDF receipts are preserved,
    which is critical for expense tracking and documentation. This delegates to
    the same forwarding path as send_gmail_message(forward_message_id=...), so
    ...
    Args:
        ...
        to (str): Recipient email address.
        subject (Optional[str]): Overrides the auto-derived "Fwd: <original>" subject.
        body (Optional[str]): Optional note prepended above the forwarded content.
        body_format (Literal["plain", "html"]): Format of the note. Defaults to "plain".
        include_attachments (bool): Forward the original attachments. Defaults to True.
        cc (Optional[str]): Optional CC recipients.
        bcc (Optional[str]): Optional BCC recipients.
